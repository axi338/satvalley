                                        ...
                                        <table className="w-full text-left text-sm">
                                            ...
                                            <tbody className="divide-y divide-white/5">
                                                {registrations.length === 0 ? (
                                                    <tr>
                                                        <td colSpan={3} className="p-8 text-center text-zinc-500">No registrations found.</td>
                                                    </tr>
                                                ) : (
                                                    registrations.map((reg) => (
                                                        <tr key={reg.id} className="hover:bg-white/5 transition-colors">
                                                            <td className="p-4 font-medium text-white">{reg.user_email || reg.user_id}</td>
                                                            <td className="p-4 text-zinc-400 font-mono text-xs">{reg.test_id.slice(0, 8)}</td>
                                                            <td className="p-4 text-zinc-500">{new Date(reg.registered_at).toLocaleDateString()}</td>
                                                        </tr>
                                                    ))
                                                )}
                                            </tbody>
